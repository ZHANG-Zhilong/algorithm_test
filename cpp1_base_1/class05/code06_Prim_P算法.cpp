...
public:
    struct edgeEqual {
        bool operator()(const Edge &e, const Edge &d) const {
    ...
    set<Edge> primMST(const Graph &graph) {
        ...
        priority_queue<Edge, edgeEqual> priorityEdges;
        unordered_set<Node, nodeHash, nodeEqual> nodeSet;
        set<Edge> rst;

        for (auto n : graph.nodes) {
            if (nodeSet.count(n.second) != 0) {
                Node node = n.second;
                nodeSet.insert(node);

                for (auto e : node.edges) {
                    priorityEdges.push(e);
                }

                while (!priorityEdges.empty()) {
                    Edge minEdge = priorityEdges.top();
                    priorityEdges.pop();
                    Node toNode = *minEdge.to;
                    if (nodeSet.count(toNode) == 0) {
                        nodeSet.insert(toNode);
                        rst.insert(minEdge);
                        for (Edge nextEdges : toNode.edges) {
                            priorityEdges.push(nextEdges);
                        }
                    }
                }
            }
